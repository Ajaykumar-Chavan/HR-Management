...
alter table employees add new_salary decimal(8,2)
...
/* 6)WRITE A QUERY FOR:- */
alter table courses add credit_hours decimal(2,2)
alter table courses add fee varchar(50),max_limit varchar(50)
alter table courses alter column  fee varchar(50) not null
alter table courses drop fee
alter table courses drop max_limit,credit_hours

/* 6)WRITE A QUERY FOR:- */
alter table project_milestones
add constraint project_milestones_table
foreign key (project_id)
references project (project_id)
alter table project_milestones
add constraint project_milestone_table
foreign key (project_id)
references project(project_id)
 ...
 */
 ...
 where salary > all 
 (select avg(salary) from employees emp where emp.department_id = e.department_id)
 select * from employees
...
VALUES
 ...
 select fruit_name,basket_name from fruits
 full outer join baskets
 on fruits.basket_id = baskets.basket_id
 where fruit_name is not null or basket_name is not  null 
 select basket_name from baskets b
...
group by department_id
select e.department_id,count(e.department_id)from employees e
join departments
on e.department_id =departments.department_id
group by e.department_id
select department_id, count(*)'headcount' from employees
group by department_id
order by headcount
select department_id,emp_count from (
select department_id,count(*)'emp_count' from employees
group by department_id) as dep_count
where emp_count >5
...
group by department_id
select department_id,job_id,count(*)'row_count' from employees
group by department_id,job_id

/* SQL HAVING clause */
select M.First_Name as ManagerFirstName, M.Last_Name as ManagerLastName, 
       E.First_Name as EmployeeFirstName, E.Last_Name as EmployeeLastName,
       COUNT(D.Dependent_ID) as DirectReportsCount
from employees M
inner join employees E on M.Employee_ID = E.Manager_ID
left join dependents D on E.Employee_ID = D.Employee_ID
group by M.Employee_ID, M.First_Name, M.Last_Name, E.Employee_ID, E.First_Name, E.Last_Name
having COUNT(D.Dependent_ID) > 0
order by M.Employee_ID, E.Employee_ID;
select m.employee_id as manager_id,m.first_name as manager_first_name,m.last_name as manager_last_name from employees m
left join employees e on m.employee_id = e.manager_id
group by m.employee_id,m.first_name,m.last_name
having count(m.employee_id) >=5
select department_id,sum(salary)'salary'from employees
group by department_id
having sum(salary) between 20000 and 30000
select employees.department_id,department_name,min(salary)'min_salary' from employees
left join departments on employees.department_id = departments.department_id
group by employees.department_id,department_name,salary
...
case
when YEAR(GETDATE()) - YEAR(hire_date) = 0 then 'Work Anniversary This Year'
when YEAR(GETDATE()) - YEAR(hire_date) = 1 then '1 Year Anniversary'
when YEAR(GETDATE()) - YEAR(Hire_Date) > 1 then CAST(YEAR(GETDATE()) - YEAR(hire_date) as nvarchar(255)) + ' Year Anniversary'
else 'Not Applicable'
...
SQL SUBQUERY */
select employee_id,first_name,last_name,d.department_id,department_name,location_id from employees e
left join departments d
on e.department_id = d.department_id
where location_id in 
(select location_id from departments
where location_id =1700)
select employee_id,first_name,last_name,d.department_id, location_id from employees e
...
group by department_id
select avg(average_salary) as avg_sal from 
(select department_id, avg(salary) as average_salary from employees group by department_id
) AS department_averages;
select * from employees
join(select avg(salary) as avg_sal from employees)
A on 1=1
